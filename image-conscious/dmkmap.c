/*
 * dmkmap - to be used in conjunction with (extends/requires) dmklib source 
 *   hastily cobbled together from bits of source included in that distribution
 *
 * dumps a text map of good/faulty sectors within a .dmk format disk image file
 *
 *
...
int main (int argc, char *argv[])
{
  ...
  if (argc != 2)
    {
      fprintf (stderr, "usage: %s image.dmk\n", argv [0]);
      exit (1);
    }
...
#endif
        printf("  %d(%d)\t", sector_info[sector_index[sector]].sector, sector_info[sector_index[sector]].size_code);
	      }
	    else
	      /* printf ("error reading cyl %d head %d sector %d size code %d\n",
		      ...
		      sector_info [sector_index [sector]].sector,
		      sector_info [sector_index [sector]].size_code); */
		  printf("X:%d(%d)\t", sector_info[sector_index[sector]].sector, sector_info[sector_index[sector]].size_code);
	  }
	  printf("\n");
